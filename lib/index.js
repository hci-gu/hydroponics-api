...
app.post('/image', (req, res) => {
  ...
  s3.putObject(params, async (err, data) => {
    ...
    try {
      await Image.create({
        imageUrl: `https://${BUCKET_NAME}.s3.${REGION}.amazonaws.com/original/${fileName}`,
...
app.get('force-migration', (req, res) => {
  const images = await Image.findAll({})
  await Promise.all(images.map(image => {
    image.imageUrl = image.imageUrl.replace('amazonaws.com', 'amazonaws.com/original')
    return image.save()
  }))
})
